...
client.on('message', (channel, user, message, self) => {
  ...
  if ((user.mod || user.username === channel.replace('#', '')) && message.startsWith('+com')) {
    ...
    dbCommands.run('INSERT INTO comandos_chat (comando, resposta) VALUES (?, ?)', [command, response], (err) => {
      if (err) {
        client.say(channel, `Erro ao adicionar o comando: ${err.message}`);
        return;
      }
  ...
  if (message == '!msg') {
    enviarMensagemDiscord('Olá, isso é uma mensagem do meu bot da Twitch!');
    client.say(channel, `@${user.username}, seus pontos foram convertidos em carlinhos coins!!!`)
  }
            ...
            db.run('UPDATE points SET points = ? WHERE username = ?', newPoints, userString.toLowerCase(), (err) => {
              if (err) {
                ...
                const mailOptions = {
                  ...
                  text: `Corpo do e-mail. Seu número é ${desiredId}. Descrição: ${idTable.find((entry) => entry.id === desiredId)?.description || 'Sem descrição'
  ...
  if (message.toLowerCase().startsWith('!sorte')) {
    ...
    if (cupOptions.includes(userChoice)) {
      if (userChoice === correctCup) {
        // Usuário acertou e ganha 50 pontos
        db.get('SELECT * FROM points WHERE username = ?', username, (err, row) => {
          if (err) {
            console.error(err.message);
          }
          const currentPoints = (row ? row.points : 0) + 50;
          db.run('INSERT OR REPLACE INTO points (username, points) VALUES (?, ?)', username, currentPoints, (err) => {
            if (err) {
              console.error(err.message);
            } else {
        ...
        // Usuário errou e perde 10 pontos
        db.get('SELECT * FROM points WHERE username = ?', username, (err, row) => {
          if (err) {
            console.error(err.message);
          }
